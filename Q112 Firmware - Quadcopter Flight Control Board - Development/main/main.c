...
//		ESC#1 to PortC.0 => PWM Output...PWF0D
//		ESC#2 to PortB.2 => PWM Output...PWED
//		ESC#3 to PortA.0 => PWM Output...PWDD
//		ESC#4 to PortA.1 => PWM Output...PWCD
//
// Sensors:
...
// I/O PIN DATA ALIASES...
	...
	// SPI is Not used on the Q112... Thus, I'm taking the MISO pin and dedicating it to the AccGyro Interrupt
	...
	//MISC I/O Pins
		...
		//#define AccGyr_STAT	PC7D	//	I/O Gyro Interrupt Pin (orig assignment to PC7D, but we need external interrupt functionality)
		#define AccGyr_STAT	PB3D	//Set for EXI7
...
static unsigned char			AccGyro_GyroAccelLPF_SMPRTDIVContents = 0x0B;	//	Changing SMPRT to 11
...
static unsigned char			AccGyro_ReadFlag = 0;
static unsigned int				AccGyro_MissedIntCount = 0;
...
static float					Accel_Xcal[3];
static float					Accel_Ycal[3];
static float					Accel_Zcal[3];
static float					Gyro_Xcal = 0;
static float					Gyro_Ycal = 0;
static float					Gyro_Zcal = 0;
...
static float					Accel_PID_XRoll_kp = 5;			//Calibrated 3-30-2014	//0.8
static float					Accel_PID_XRoll_ki = 0;							//0.001
static float					Accel_PID_XRoll_kd = 0;							//0.5
static float					Accel_PID_YPitch_kp = 5;
...
static unsigned char			Accel_PID_GoCounter = 0;
...
#ifdef DEBUG_DD
static unsigned int				KeyCFData[4][100];
//static unsigned int 			KeyMOTData[4][100];
static unsigned int				ArrayCounter = 0;
#endif

#ifdef DEBUG_STEP
static unsigned char			MotorStepCount = 0;
static unsigned int				MotorSpeedUT = 0;
...
static unsigned int 			PWMUpperDutyLimitRun = 10500;			//Value for Maximum Duty	//18min flight = 11500
#ifndef DEBUG_STEP
static unsigned int 			PWMLowerDutyLimitRun = 9500;			//Value for Minimum Duty	//18min flight = 9500
#endif																		//9000 duty 	=  	3040rpm
#ifdef DEBUG_STEP
static unsigned int 			PWMLowerDutyLimitRun = 8000;			//Value for Minimum Duty	//18min flight = 9500
#endif																		//10000 duty 	= 	4580rpm
																		//11000 duty	=	5820rpm
																		//12000 duty	=	6700rpm
																		//13000 duty	=	7780rpm
...
Fast_Loop:							//This loop takes 22.4ms for this loop as of 3/30/2014		
		...
		/*		
		if(Accel_PID_XRollCounter>50){		//Counter++ @ 125us, need 7ms, thus 7*125us = 14ms //Counter Reset in Accel/Gyro PID Loop and incremented in TMR89_ISR
			//Get_AccGyroData();	//Moved outside of loop			
			//ReturnAccelData();	//Don't run this normally, UART takes too much time...
			LED_3 = 1;
		...
		*/
		...
		if(AccGyro_ReadFlag >= 1){		//Triggered by External Interrupt (flag set in AccelGyroDataReady_ISR)
			AccGyro_ReadFlag = 0;
			//LED_4 = 1;			//C2, Pin 14	//Inside GET_AccGyro
			Get_AccGyroData();
			//LED_4 = 0;
		}
...
static void Initialization(void){
	...
	*/
	...
	PB3MD0 = 0;
	PB3E1 = 1;
	//PB3E1 = 0;	//Trying to enable this last....
	PB3E0 = 0;		//PBnE0-1 are used to choose the Rising-Edge Mode for this interrupt
	PB3SM = 0;
	//Setup External Interrupt Handler for External Interrupt connected to Accel/Gyro
	(void)irq_setHdr( (unsigned char)IRQ_NO_PB3INT, AccelGyroDataReady_ISR );  //Clear interrupt request flag
	//(void)irq_setHdr( (unsigned char)IRQ_NO_PB3INT, Get_AccGyroData);  //Clear interrupt request flag
	...
	//----- Gyro/Accel Turn On Data Ready Interrupt
	_flgI2CFin = 0;														//reset I2C completed flag
	i2c_stop();															//Make sure I2C is not currently running
...
int i;
	...
	for(i = 0; i<10; i++){
		...
		//EPB3 = 1;
		//PB3E1 = 1;
		//Since Gyro is FLAT, Gravity only effects X and Y.  Thus, only save X and Y
	...
	//Start Receive and wait for the return message
	_flgUartFin = 0;
	uart_stop();
	uart_startReceive(RecWorld, 2, _funcUartFin);
	while(_flgUartFin != 1){
	...
	for(i = 0; i<10; i++){
		...
		//EPB3 = 1;
		//PB3E1 = 1;
		//Since Gyro is ROLL +90deg, Gravity only effects Y and Z.  Thus, only save X and Y
		Accel_Zcal[1] += (AccGyro_Data[4]<<8)+(AccGyro_Data[5]);
		Accel_Ycal[2] += (AccGyro_Data[2]<<8)+(AccGyro_Data[3]);
	}
	Accel_Zcal[1] /= 10;
	Accel_Ycal[2] /= 10;
	//----- Part 3: Calibrate Against PITCH -----
	//Send Message Indicating Calibration to Begin
	for(i = 0; i<150; i++)
	{
		SensorReturn[i] = 0x20;
	...
	//Average both sets of Calibration for X, Y, Z
		...
		if(RecWorld[0] == 0x44){			//if RECWORLD == "DD" for Data Dump
			if(RecWorld[1] == 0x44){
                ...
                for (j = 0; j < 100; j++){
                    ...
                    sprintf(SensorReturn, "%u,%u,%u,%u,%u", index,KeyCFData[0][j],KeyCFData[1][j],KeyCFData[2][j],KeyCFData[3][j]);
					SensorReturn[198] = 0x0D;
					SensorReturn[199] = 0x0A;
					//Send Returned Sensor Output to PC!
					_flgUartFin = 0;
...
void AccelSensorControlPID(void){
	...
	//else{
		...
		*/
		...
		//  -------------------------------------------------------  STEP TEST --------------------------------------------------------------------------------------------------------------
		// Step Test Logic	
		/* //Old Logic... doesn't work cause DD is not fast enough
		if(MotorStepCount < 1){
			MotorStepCount++;		
			PFRUN = 0;	//Turn OFF PWM
			PERUN = 0;
			PDRUN = 0;
			PCRUN = 0;
			PWF0D = 8550; 		//Can't be running to change (Only this variable)
			PWED = 8450;
			PWCD = 8450;	
			PWDD = 8450;
			CheckSafetyLimit();
			...
			if(MotorStepCount == 9){
				...
				PDRUN = 0;
				PCRUN = 0;
				PWF0D = 8450; 		//Can't be running to change (Only this variable)
				PWED = 8450;
				PWCD = 8450;	
				PWDD = 8450;
				CheckSafetyLimit();
		...
		*/
		...
		#ifdef DEBUG_DD
		if(ArrayCounter < 100) {
			KeyCFData[0][ArrayCounter] = (unsigned char) CF_XRoll;	//X-Data
			KeyCFData[1][ArrayCounter] = (unsigned char) CF_YPitch;	//Y-Data
			KeyCFData[2][ArrayCounter] = MotorTimerUT;					//Motor Under Test
			KeyCFData[3][ArrayCounter] = MotorSpeedUT;				//Motor PWM Value
...
//===========================================================================
// ----- ISR for External Interrupt Pin on Accel/Gyro
static void AccelGyroDataReady_ISR( void ) 
{
...
//		ESC#4 to PortA.1 => PWM Output...PWCD
	...
	if(MotorStepCount < 1){
		...
		if(MotorStepUpDnFlg == 0){
			...
			#endif
			#ifdef DEBUG_STEP_TESTY
			PWF0D = StepTestNominal+MotorSpeedChange; 		//Can't be running to change (Only this variable)
			PWED = StepTestNominal+MotorSpeedChange;
			PWDD = StepTestNominal;
			PWCD = StepTestNominal;	
			#endif
			#ifdef DEBUG_STEP_TESTY_Inv
			PWF0D = StepTestNominal; 						//Can't be running to change (Only this variable)
			PWED = StepTestNominal;
			PWDD = StepTestNominal+MotorSpeedChange;
			PWCD = StepTestNominal+MotorSpeedChange;	
