...
static float					Accel_PID_XRoll_kp = 1.6;		//Calibrated 3-30-2014	//0.8
static float					Accel_PID_XRoll_ki = 0.0004;							//0.001
static float					Accel_PID_XRoll_kd = 1.75;								//0.5
static float					Accel_PID_YPitch_kp = 1.6;
static float					Accel_PID_YPitch_ki = 0.0004;
static float					Accel_PID_YPitch_kd = 1.75;

unsigned char					Accel_PID_XYChangeFlag = 0;
static unsigned int				Accel_PID_Counter = 0;

static unsigned int				Accel_PID_Motor1 = 32767;
static unsigned int				Accel_PID_Motor2 = 32767;
static unsigned int				Accel_PID_Motor3 = 32767;
static unsigned int				Accel_PID_Motor4 = 32767;
...
void Get_AccGyroData(void){
	...
	static unsigned int		CF_Accel_XRoll_FixedPoint = 0;
	static unsigned int		CF_Accel_YPitch_FixedPoint = 0;
	static unsigned int		CF_Gyro_XRoll_FixedPoint = 0;
	static unsigned int		CF_Gyro_YPitch_FixedPoint = 0;
	static unsigned int		CF_Accel_ForceMagApprox_FixedPoint = 0;
	static unsigned int		CF_Gyro_CurrentCount_FixedPoint = 0;
LED_2 = 1;
...
/* 
=========================================================================
COMMENT OUT Floating Point Math to see if we can speed up this routine!
=========================================================================
...
//This section with Fixed Point math takes Xms as of 3/30/2013
//Local
	//static unsigned int		CF_Accel_XRoll_FixedPoint = 0;
	//static unsigned int		CF_Accel_YPitch_FixedPoint = 0;
	//static unsigned int		CF_Gyro_XRoll_FixedPoint = 0;
	...
	//static unsigned int		CF_Gyro_CurrentCount_FixedPoint = 0;
//Global
	static unsigned int			CF_Gyro_Counter = 0;
	static unsigned int			CF_YPitch = 0;
	static unsigned int			CF_XRoll = 0;
	static unsigned int			CF_HPF = 97; 		// = a = tau/(tau+dt) => Calibrated 3-28-2014 = 0.97 
	static unsigned int			CF_LPF = 3;			// = 1-a

	
	//----- Start Complementary Filter for Sensor Fusion -----
	CF_Gyro_CurrentCount_FixedPoint = CF_Gyro_Counter_FixedPoint / 1000;		
	CF_Gyro_Counter_FixedPoint = 0;
	
	CF_Gyro_YPitch_FixedPoint = CF_YPitch + ((Gyro_Xout/AccGyro_GyroScaling) * CF_Gyro_CurrentCount);
	CF_Gyro_XRoll = CF_XRoll - ((Gyro_Yout/AccGyro_GyroScaling) * CF_Gyro_CurrentCount);	
...
void AccelSensorControlPID(void){
	...
	static unsigned int				Accel_PID_Motor1_temp = 0;
	static unsigned int				Accel_PID_Motor2_temp = 0;
	static unsigned int				Accel_PID_Motor3_temp = 0;
	static unsigned int				Accel_PID_Motor4_temp = 0;
	...
	if(Accel_PID_XYChangeFlag == 0){
		...
		*/
		...
		Accel_PID_Motor1_temp = Accel_PID_Motor1 * 0.2594;	//Conversion from 65535 Scaling to PWM Period of 17000
		Accel_PID_Motor2_temp = Accel_PID_Motor2 * 0.2594; 
		Accel_PID_Motor3_temp = Accel_PID_Motor3 * 0.2594; 
		Accel_PID_Motor4_temp = Accel_PID_Motor4 * 0.2594; 
...
void CheckSafetyLimitUnscaled(void){	
	//Motor Safety Limit
	if(Accel_PID_Motor1 > (PWMUpperDutyLimitRun*3.855)){
		Accel_PID_Motor1 = PWMUpperDutyLimitRun*3.855;
	}
	if(Accel_PID_Motor1 < (PWMLowerDutyLimitRun*3.855)){
		Accel_PID_Motor1 = PWMLowerDutyLimitRun*3.855;
	}
	if(Accel_PID_Motor2 > (PWMUpperDutyLimitRun*3.855)){
		Accel_PID_Motor2 = PWMUpperDutyLimitRun*3.855;
	}
	if(Accel_PID_Motor2 < (PWMLowerDutyLimitRun*3.855)){
		Accel_PID_Motor2 = PWMLowerDutyLimitRun*3.855;
	}
	if(Accel_PID_Motor3 > (PWMUpperDutyLimitRun*3.855)){
		Accel_PID_Motor3 = PWMUpperDutyLimitRun*3.855;
	}
	if(Accel_PID_Motor3 < (PWMLowerDutyLimitRun*3.855)){
		Accel_PID_Motor3 = PWMLowerDutyLimitRun*3.855;
	}
	if(Accel_PID_Motor4 > (PWMUpperDutyLimitRun*3.855)){
		Accel_PID_Motor4 = PWMUpperDutyLimitRun*3.855;
	}
	if(Accel_PID_Motor4 < (PWMLowerDutyLimitRun*3.855)){
		Accel_PID_Motor4 = PWMLowerDutyLimitRun*3.855;
	}
}
...
void Shutdown(void){
	...
	Accel_PID_Motor1 = 32767;
	Accel_PID_Motor2 = 32767;
	Accel_PID_Motor3 = 32767;
	Accel_PID_Motor4 = 32767;
...
static void TMR89_ISR( void ) 
{
	...
	CF_Gyro_Counter_FixedPoint++;
	if(CF_Gyro_Counter_FixedPoint >= 65535){
		CF_Gyro_Counter_FixedPoint = 0;
	}
}
//===========================================================================
